...
namespace IEXTrading.Infrastructure.IEXTradingHandler
{
    public class IEXHandler
    {
        ...
        ****/
        ...
        {
            string IEXTrading_API_PATH = BASE_URL + "/stock/market/collection/list?collectionName=infocus";
            string companyList = "";
            ...
            if (!companyList.Equals(""))
            {
                ...
                companies = companies.GetRange(0, 30);
            }
            return companies;
        }
        ...
        ****/
        ...
        public List<Headline> GetHeadlines()
        {
            ...
            if (response.IsSuccessStatusCode)
            {
                headlineList = response.Content.ReadAsStringAsync().GetAwaiter().GetResult();
            }

            if (!headlineList.Equals(""))
            {
                headlines = JsonConvert.DeserializeObject<List<Headline>>(headlineList);
                if(headlines.Count >= 5)
                {
                    headlines = headlines.GetRange(0, 5);
                }
            }
            return headlines;
        }
